...
local SHOW_TEAM = true -- team color, uselses for pd
local SHOW_DISTANCE = 70000
...
local function isLookingAt(pos)
...
local function updateESP()
	for name, data in pairs(playerBoxes) do
		...
		local looking = isLookingAt(root.Position)
			...
			if not p1 or not p2 then
				...
				if on1 and on2 then
					...
					if looking then
						...
						local color = plr.Team and plr.Team.TeamColor.Color or Color3.fromRGB(255,255,255)
						if plr.Team == Players.LocalPlayer.Team and plr.Team ~= nil then
							seg.line.Color = Color3.fromRGB(120,120,120)
						else
